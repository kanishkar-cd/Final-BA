...
export const Sidebar: React.FC = () => {
  ...
  const filteredDockItems = filteredSteps.map((step) => {
    ...
    return {
      ...
      label: step.label,
      onClick: () => router.push(`/projects/${projectId}/${step.path}`),
  ...
  return (
    <div className="flex h-screen select-none shrink-0 z-30 bg-background border-r border-border transition-all duration-500 relative" style={{ width: isExpanded ? '280px' : '80px' }}>
      ...
      <div className={`flex flex-col h-full w-full py-4 ${isExpanded ? 'overflow-hidden' : 'overflow-visible'} relative`}>
        ...
        {projectId && (
          <div className={`flex-1 overflow-y-auto ${isExpanded ? 'overflow-x-hidden' : 'overflow-x-visible'} px-1 py-2 relative hide-scrollbar`}>
            {isExpanded ? (
              <>
